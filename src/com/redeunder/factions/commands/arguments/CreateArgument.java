...
public class CreateArgument extends Argument {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (UnderFactions.getInstance().factionManager.isAlreadyTag(tag)) {
            player.sendMessage("§cJá existe uma facção com a tag " + tag + ".");
            return;
        }

        if (UnderFactions.getInstance().factionManager.isAlreadyName(name)) {
            player.sendMessage("§cJá existe uma facção com o nome " + name + ".");
            return;
        }

        if (UnderFactions.getInstance().factionManager.containsSpecialCharacter(tag)) {
            player.sendMessage("§cA tag da sua facção contém caractéres inválidos.");
            return;
        }

        if (UnderFactions.getInstance().factionManager.containsSpecialCharacter(name)) {
        ...
        UnderFactions.getInstance().factions.put(name, faction);
